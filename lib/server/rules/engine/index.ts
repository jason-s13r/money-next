...
import { tagTransactions } from "../../labels";
...
export async function runRules(
  ...
  try {
    ...
    for (const tx of txs) {
      ...
      try {
        ...
        // Only a tag the rule was configured to apply. The `category-rule-<slug>`
        // tags this used to derive are gone: they named an effect, accumulated
        // across every run that ever had it, and the run report answers the
        ...
        const own =
          made.length > 0 && typeof output.labelName === "string"
            ? output.labelName.trim()
            : "";
        if (own) {
          const ids = wanted.get(own);
          if (ids) ids.push(tx.id);
          else wanted.set(own, [tx.id]);
        }
      } catch {
        summary.errors++;
      }
  ...
  for (const [name, ids] of wanted) {
    try {
      for (const transactionId of await tagTransactions(db, name, ids)) {
