...
// `Transaction.categoryGroupId` points at the row's `personal_finance` group
// (its name lives on `CategoryGroup`).
//
...
//     at all. That is why a real spending group is absent on every inflow, and
//     why a non-null spending `categoryGroupId` is a faithful test for "this is
//     spending" rather than a lucky heuristic.
//
...
// The same two income groups by their `CategoryGroup` id (see lib/server/nzfcc.ts),
// for the metrics that now group and filter transactions by `categoryGroupId`
// rather than by the group name.
export const PERIODIC_INCOME_GROUP_ID = "group_custom_periodic_income";
export const OTHER_INCOME_GROUP_ID = "group_custom_other_income";
export const INCOME_GROUP_IDS = [PERIODIC_INCOME_GROUP_ID, OTHER_INCOME_GROUP_ID] as const;

export function isIncomeGroupId(id: string): boolean {
  return (INCOME_GROUP_IDS as readonly string[]).includes(id);
}
