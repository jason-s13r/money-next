...
import { CELL, CHEVRON, HEAD, formatCell, type CellFormat } from "../comparison-table";
      ...
      <table className="w-full min-w-3xl text-sm">
        ...
        <tbody>
          ...
          {incomeNodes(comparison).map((node) => (
            <SpendRow key={node.label} row={node} format={format} />
          ))}
          <TotalRow label="Total income" values={periods.map((p) => p.incomeTotal)} format={format} />
          ...
          {spendCategories.map((category) => (
            <SpendRow key={category} row={spendNode(comparison, category)} format={format} />
          ))}
          <TotalRow label="Total spending" values={periods.map((p) => p.spendTotal)} format={format} />
